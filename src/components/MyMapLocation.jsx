...
function MyMapLocationLayout({ folder, children, onEditModalOpen, isLocationList = false }) {
      ...
      {isLocationList && (
        <header className={styles.header}>
          <span className={styles['header-location']}>
            <span className={styles.label}>📍 {isKorean ? '저장된 장소' : 'Saved Locations'}</span>
            <span className={styles.counts}>{isKorean ? `${locationCounts}곳` : `${locationCounts} places`}</span>
          </span>
          <span className={styles['header-action']}>
            <button className={styles.edit} onClick={() => onEditModalOpen(true)}>
              <ModifyIcon /> {isKorean ? '폴더 수정' : 'Edit folder'}
            </button>
...
function MyMapLocation({ maps, selectedFolder = [], selectedFolderId, onMaps }) {
  ...
  if (!folder?.location_id.length)
    return (
      <>
        ...
        <MyMapLocationLayout folder={folder} onEditModalOpen={setEditModalOpen} isLocationList>
